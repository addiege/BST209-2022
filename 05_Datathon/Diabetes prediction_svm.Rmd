---
title: "BST209 Group Project Team G - Support Vector Machine"
...
forest <- svm(outcome_variable ~ age + bmi + gender + glucose_apache + h1_glucose_max + d1_glucose_max + ethnicity + wbc_apache, probability=TRUE,data = train_set) 
...
forest_probs <- predict(forest, test_set, probability = TRUE)
forest_probs <- attr(forest_probs, "probabilities")[,1]
...
```{r}
forest_pred_orig <- predict(forest,
                       newdata = original_test_set,
                       type = "class")

# Get the probabilities for our forest predictions
forest_probs_orig <- predict(forest, original_test_set, probability = TRUE)
forest_probs_orig <- attr(forest_probs_orig, "probabilities")[,1]

outcome <- original_test_set$outcome_variable
```
...
for (i in list)
{
forest_probs <- forest_probs_orig[original_test_set[[var]] == i]
forest_pred <- forest_pred_orig[original_test_set[[var]] == i]
outcome_sub <- outcome[original_test_set[[var]] == i]

num = length(forest_probs)

# Make predictions on test set

# Create a "prediction" object using our probabilities and the outcome variable
forest_roc <- prediction(forest_probs, outcome_sub)
forest_perf <- performance(forest_roc, measure = "tpr", x.measure = "fpr")
# Plot the ROC curve
...
if(i==1){
  forest_probs <- forest_probs_orig[original_test_set[[var]] < var_range[i]]
  forest_pred <- forest_pred_orig[original_test_set[[var]] < var_range[i]]
  outcome_sub <- outcome[original_test_set[[var]] < var_range[i]]
}
else if(i==length(var_range)+1){
  forest_probs <- forest_probs_orig[original_test_set[[var]] > var_range[i-1]]
  forest_pred <- forest_pred_orig[original_test_set[[var]] > var_range[i-1]]
  outcome_sub <- outcome[original_test_set[[var]] > var_range[i-1]]
}
else{
  test_set <- original_test_set[original_test_set[[var]] > var_range[i-1] & original_test_set[[var]] < var_range[i], ]
  forest_probs <- forest_probs_orig[original_test_set[[var]] > var_range[i-1] & original_test_set[[var]] < var_range[i]]
  forest_pred <- forest_pred_orig[original_test_set[[var]] > var_range[i-1] & original_test_set[[var]] < var_range[i]]
  outcome_sub <- outcome[original_test_set[[var]] > var_range[i-1] & original_test_set[[var]] < var_range[i]]
}
